...
print("Start installing vilib %s for user %s"%(__version__ ,user))
...
def install():
    ...
    print("Create workspace and video_control")
    if not os.path.exists('/opt'):
        os.mkdir('/opt')
        run_command('chmod 774 /opt')
        run_command(f'chown -R {user}:{user} /opt')
    do(msg="create dir",
        cmd='mkdir -p /opt/vilib'
        + ' && chmod 774 /opt/vilib'
        + f' && chown -R {user}:{user} /opt/vilib'
        )
