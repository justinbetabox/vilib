...
traffic_sign_model_path = "/opt/vilib/tf_150_dr0.2.tflite"    
...
def ProcessCommands():
    if request.args.get('command') == 'forward': #type: ignore
        px.forward(px.current_speed)
    elif request.args.get('command') == 'backward': #type: ignore
        px.backward(px.current_speed)
        ...
        px.set_cam_pan_angle(-10)
    elif request.args.get('command') == 'change_color':
        Vilib.set_color(request.args.get('value'))
    elif request.args.get('command') == 'color_detect': #type: ignore
        if Vilib.detect_obj_parameter['cdf_flag'] == False:
            Vilib.color_detect_switch(True)
        else:
            Vilib.color_detect_switch(False)
    elif request.args.get('command') == 'human_face_detect': #type: ignore
        if Vilib.detect_obj_parameter['hdf_flag'] == False:
            Vilib.human_detect_switch(True)
        else:
            Vilib.human_detect_switch(False)
    elif request.args.get('command') == 'speedup': #type: ignore
        if px.current_speed <= 90:
            if px.current_direction < 0:
                px.set_speed(px.current_speed + 10)
                px.backward(px.current_speed + 10)
            elif px.current_direction > 0:
                px.set_speed(px.current_speed + 10)
                px.forward(px.current_speed + 10)
            else:
                px.set_speed(px.current_speed + 10)
    elif request.args.get('command') == 'speeddown': #type: ignore
        if px.current_speed > 10:
            if px.current_direction < 0:
                px.set_speed(px.current_speed - 10)
                px.backward(px.current_speed - 10)
            elif px.current_direction > 0:
...
# detect_obj_parameter
    ...
    detect_obj_parameter['object_t'] = 'None'      
    ...
    # Perform the actual detection by running the model with the image as input
        image_np_expanded = im5.astype('float32') 
                    ...
                    if w > 32 and h > 32: 
                        ...
                        if acc_val >= 75:   
                            if traffic_type == 1 or traffic_type == 2 or traffic_type == 3:
                                ...
                                red_mask_1 = cv2.inRange(hsv[y:y+h,x:x+w],(0,50,20), (4,255,255))           
                                ...
                                open_img = cv2.morphologyEx(red_mask_all, cv2.MORPH_OPEN,Vilib.kernel_5,iterations=1)             
                                ...
                                # print("contours:",contours_count)
                                    ...
                                    if corners >= 0:
                                        ...
                                        cv2.putText(img,str(traffic_dict[traffic_type]) +': ' + str(round(acc_val)),(x,y), cv2.FONT_HERSHEY_SIMPLEX, 1,(255,0,255),2)
                                        if w * h > max_area:
                                            max_area = w * h
                                            max_obj_x = x
                                            max_obj_y = y
    ...
    def color_detect_func(img):
        ...
        if Vilib.detect_obj_parameter['cdf_flag']  == True:
            ...
            mask = cv2.inRange(hsv,np.array([min(Vilib.color_dict[color_type]), 60, 60]), np.array([max(Vilib.color_dict[color_type]), 255, 255]) )         
            if color_type == 'red':
                 mask_2 = cv2.inRange(hsv, (167,0,0), (180,255,255))
                 mask = cv2.bitwise_or(mask, mask_2)

            open_img = cv2.morphologyEx(mask, cv2.MORPH_OPEN,Vilib.kernel_5,iterations=1)            
            contours, hierarchy = findContours(open_img)      
